...
func (h *handler) DownloadAlbum() *cli.Command {
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			fmt.Println("queued album download")
			var (
				meta      metas
				songIds   []string
				songBytes []byte
			...
			resp, err = h.client.Get(
				h.endpoint +
					fmt.Sprintf(FETCH_RESOURCE_META, pkg.RESOURCE_ALBUM, c.Args().Get(0)),
			)
			if err == nil {
				body, _ := ioutil.ReadAll(resp.Body)
				json.Unmarshal(body, &meta)
			...
			for {
				...
				resp, err := h.client.Get(
					h.endpoint +
						fmt.Sprintf(ALBUM_STATUS, c.Args().Get(0)),
				)
				if err == nil {
					var data status
					body, _ := ioutil.ReadAll(resp.Body)
...
func (h *handler) DownloadShow() *cli.Command {
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			fmt.Println("queued show download")
			var (
				meta      metas
				songIds   []string
				songBytes []byte
			...
			resp, err = h.client.Get(
				h.endpoint +
					fmt.Sprintf(FETCH_RESOURCE_META, pkg.RESOURCE_SHOW, c.Args().Get(0)),
			)
			if err == nil {
				body, _ := ioutil.ReadAll(resp.Body)
				json.Unmarshal(body, &meta)
			...
			for {
				...
				resp, err := h.client.Get(
					h.endpoint +
						fmt.Sprintf(SHOW_STATUS, c.Args().Get(0)),
				)
				if err == nil {
					var data status
					body, _ := ioutil.ReadAll(resp.Body)
